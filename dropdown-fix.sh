...
# Create updated header.php file with fixed dropdown
...
# Create script.js file to handle the dropdown behavior
...
# Create footer.php file to include the script
cat > tmp/footer.php << 'EOF'
    </main>
...
docker exec phone-store-php-1 mkdir -p /var/www/html/public/js

# Copy the updated files to the Docker container
docker cp tmp/header.php phone-store-php-1:/var/www/html/src/Views/layouts/
docker cp tmp/footer.php phone-store-php-1:/var/www/html/src/Views/layouts/
docker cp tmp/script.js phone-store-php-1:/var/www/html/public/js/
...
echo "Dropdown menu fix applied successfully." 